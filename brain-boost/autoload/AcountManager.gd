...
signal streak_loaded(count: int)

func get_streak() -> void:
	var today = Time.get_date_string_from_system()
	_check_date_for_activity(today, 0)

func _check_date_for_activity(date: String, current_streak: int) -> void:
	# Build a range for the full day: date 00:00:00 to date 23:59:59
	var url = SUPABASE_URL + \
		"/rest/v1/scores" + \
		"?user_id=eq." + user_id + \
		"&played_at=gte." + date + "T00:00:00" + \
		"&played_at=lte." + date + "T23:59:59" + \
		"&limit=1"
	
	var http = _make_request(url, HTTPClient.METHOD_GET, _read_headers(), "")
	
	# Capture date + streak in a closure
	http.request_completed.connect(
		func(_res, code, _headers, body):
			_on_streak_date_checked(code, body, date, current_streak),
		CONNECT_ONE_SHOT
	)

func _on_streak_date_checked(response_code: int, body: PackedByteArray, date: String, current_streak: int) -> void:
	if response_code != 200:
	...
	_check_date_for_activity(prev_date, current_streak + 1)

func _date_minus_one(date: String) -> String:
	var unix_time = Time.get_unix_time_from_datetime_string(date + "T12:00:00")
	if unix_time == 0 and not date.begins_with("1970"):
		return "NA"
	var dict = Time.get_date_dict_from_unix_time(unix_time - 86400)
